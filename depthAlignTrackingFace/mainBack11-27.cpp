...
void convertToLandmarks3D(Mat &depth){
	...
	//for(i = 0; i < n; i++, dp++){   
	for(i = 0; i < n; i++){
		/*if(visiG.at<int>(i,0) == 0)continue;*/
		
		p1 = cv::Point(shapeG.at<double>(i,0),shapeG.at<double>(i+n,0));
		//imageDosLandmarks.at<uchar>(p1.y,p1.x) = 255;
		imageDosLandmarks.at<uchar>(p1.y,p1.x) = 255;
		
		/*Vector4 RealPoints = NuiTransformDepthImageToSkeleton(p1.x,p1.y,*dp, NUI_IMAGE_RESOLUTION_640x480);
		pShape.pX = RealPoints.x;
		pShape.pY = RealPoints.y;
		pShape.pZ = RealPoints.z;

		pontosSh.push_back(pShape);*/
	}
	imshow("out", imageDosLandmarks);
	//imwrite("out.jpg", imageDosLandmarks);
...
//				}
//
//				//if(y>=j && y<=totFaceHeight && x>=i && x<=totFaceWidth){
			...
			for(x = 0;x < pointCloud_XYZ.cols;x++,point++){
				...
				if(0 <= colorX && colorX < shapis.cols && 0 <= colorY && colorY < shapis.rows){	
					...
					if(shapis.at<uchar>(colorY,colorX) == 255){
						...
						cout<<cont<<" VITORIA!"<<colorX<<"  "<<colorY<<endl;
						cont++;
					}
				}
...
void convertDepth2RGBCoord(Mat &depth, Mat &DepthinRGB){

	INuiCoordinateMapper* pMapper;//mapeia diferentes dados entre espaços de coordenadas
	...
	unsigned short* dp = (unsigned short*)depth.data;
	unsigned short* dpRGB = (unsigned short*)DepthinRGB.data;
	...
	for (int i = 0; i < width * height; i++)
       if (colorPoints[i].x >0 && colorPoints[i].x < width && colorPoints[i].y>0 &&    colorPoints[i].y < height)
            *(dpRGB + colorPoints[i].x + colorPoints[i].y*width) = *(dp + i );
...
void display(){
	...
	//convertDepth2RGBCoord(depthPure, DepthinRGB); //as coordenadas de RGB e profundidade estão diferentes, preciso colocar depth no mesmo sistema de coordenadas de RGB

	imageComLand = image.clone();

	detectaFace(imageComLand); //detecto os landmarks da face(o que me interessa é shapeG(os pontos))

	convertToLandmarks3D(DepthinRGB); //converto shapeG para pontos XYZ
